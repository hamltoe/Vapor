/* FIPS 180-4 SHA-256. Vendored rather than pulled from a crypto library so the
 * client and server compute package digests with byte-identical code. */
...
vapor_sha256_final(vapor_sha256 *c, uint8_t out[VAPOR_SHA256_DIGEST_LEN])
{
    ...
    /* 0x80 then zeroes, spilling into a second block if the length field will
    ...
    for (i = 0; i < 8; i++) {
        c->block[56 + i] = (uint8_t)(total >> (56 - 8 * i));
    }
    sha256_transform(c, c->block);
                 ...
                 uint8_t out[VAPOR_SHA256_DIGEST_LEN])
{
    vapor_sha256 c;
    vapor_sha256_init(&c);
    vapor_sha256_update(&c, data, len);
                     ...
                     char out[VAPOR_SHA256_HEX_LEN + 1])
{
    ...
    vapor_sha256_buf(data, len, digest);
    vapor_sha256_hex(digest, out);
}

int
vapor_sha256_file(const char *path, char out[VAPOR_SHA256_HEX_LEN + 1])
{
    uint8_t      digest[VAPOR_SHA256_DIGEST_LEN];
    vapor_sha256 c;
    FILE        *f;
    size_t       n;
    /* Static-sized heap-free chunk; 64 KiB keeps syscall overhead low without
     * a large stack frame. */
